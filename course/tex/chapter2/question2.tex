...
\subsubsection*{(c) Equivalentie} \todo{TODO}
\subsubsection*{(d) Deterministische transformatie}
De transformatie van een NFA naar een DFA is steeds determinisitisch vermits er een eindig aantal toestanden zijn in een NFA. Dit zorgt er voor dat het algoritme steeds eindigt. \todo{kan je niet meerdere DFA's construren uit 1 NFA?}
\subsubsection*{(e) DFA met minder toestanden dan een NFA}
Ja het is zeker mogelijk om een equivalente DFA te construeren uit een NFA die minder toestanden bevat dan die NFA. Vaak zal echter het aantal toestanden exponentieel toenemen. \todo{"...This can be performed using the powerset construction, which may lead to an exponential rise in the number of necessary states." (zie wikipedia NFA)}
Een voorbeeld van een NFA en een equivalente DFA waarbij de DFA minder toestanden heeft dan de NFA: \todo{voorbeeld toevoegen}